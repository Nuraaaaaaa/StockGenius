...
# ─────────────────────────────────────────────────────────────────────────────
# ROLE HIERARCHY
#   admin             → full access: users, inventory, analytics, alerts
#   inventory_manager → inventory CRUD + alerts + analytics (read) — no user mgmt
#   staff             → dashboard + alerts (read-only) + analytics (read-only)
...
def inventory_access_required(f):
    """Allows admin and inventory_manager to perform inventory CRUD."""
    @wraps(f)
    def decorated(*args, **kwargs):
        if "user_id" not in session:
            return redirect(url_for("login"))
...
def dashboard():
    ...
    # Inventory managers get their own focused dashboard
    ...
    return render_template("dashboard.html", active_page="dashboard",
                           user_name=session.get("user_name"), user_role=role)
...
# ── Inventory API — read endpoints (all logged-in users) ─────────────────────
...
# ── Inventory API — write endpoints (admin + inventory_manager only) ──────────
...
# ── Stock-level update (inventory_manager shortcut) ───────────────────────────

@app.route("/api/products/by-name/<path:sub_category>/restock", methods=["POST"])
@inventory_access_required
def restock_product(sub_category):
    """
    Lightweight endpoint: update only stock_level and reorder_point.
    Used by the Inventory Manager quick-restock widget.
    """
    ...
    try:
        existing = db.run_query(conn,
            "SELECT id FROM manual_products WHERE sub_category = %s LIMIT 1", (sub_category,))
        if existing:
            db.execute_update(conn,
                "UPDATE manual_products SET stock_level=%s, reorder_point=%s WHERE sub_category=%s",
                (stock_level, reorder_point, sub_category))
        else:
            # patch ml_inventory-sourced item by inserting a shadow row
            row = db.run_query(conn, """
                SELECT MAX(category) AS cat FROM ml_inventory
                WHERE sub_category = %s LIMIT 1
            """, (sub_category,))
    ...
    try:
        ...
        for row in warning_stock_rows:
            alerts_data.append({
                ...
                "subtitle": f"{row['sub_category']}  ·  {row['region'] or ''}",
                "message": (f"{row['sub_category']} stock ({int(row['stock_level'])} units) is below the "
        ...
        for row in ml_rows:
            alerts_data.append({
                ...
                "message": (f"Random Forest model predicts continued high demand for {row['sub_category']}. "
...
# ── Inventory Manager KPI endpoint ───────────────────────────────────────────

@app.route("/api/inventory_manager/kpis")
@login_required
def inventory_manager_kpis():
    """
    Dedicated KPI endpoint for the Inventory Manager dashboard.
    Returns stock health, near-expiry counts, and top restocking priorities.
    """
    if session.get("role") not in ("admin", "inventory_manager"):
...
def alerts_live():
    ...
    try:
        rows = db.run_query(conn, """
            ...
            FROM manual_products
            WHERE (stock_level IS NOT NULL AND reorder_point IS NOT NULL
        ...
        for r in rows:
            ...
            if sl < rp and sl <= rp * 0.5:
                ...
                deficit = max(0, rp - sl); pct = round((sl / max(rp, 1)) * 100)
                message = (f"{name} has only {sl} units — {pct}% of the reorder threshold "
                           f"({rp}). Order at least {deficit} units immediately.")
                action = "Reorder Now"
            elif dte <= 7:
                alert_type = "critical"; title = "Urgent — Expiry Within 7 Days"; icon = "🗓️"
                message = (f"{name} expires in {dte} day(s). "
                ...
                message = (f"{name} will expire in {dte} days. "
                           f"Stock: {sl} units across {int(r['quantity'] or 0)} orders. "
                           f"Review pricing or schedule clearance promotion.")
                action = "Review Item"
            else:
                alert_type = "warning"; title = "Low Stock Alert"; icon = "⚠️"
                message = (f"{name} stock ({sl} units) is below the reorder point of "
            ...
            alerts.append({
                ...
                "days_to_expiry": dte,
                "quantity": int(r["quantity"] or 0), "sales": float(r["sales"] or 0),
                "profit": float(r["profit"] or 0), "avg_discount": float(r["avg_discount"] or 0),
                "region": r["region"] or "",
            })
